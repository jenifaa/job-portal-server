  ...
  try {
    ...
    app.get("/job-application", async (req, res) => {
      ...
      for (const application of result) {
        ...
        const applyJob = await jobsCollection.findOne(query1);
        if (applyJob) {
          application.title = applyJob.title;
          application.company = applyJob.company;
          application.company_logo = applyJob.company_logo;
          application.location = applyJob.location;
          application.category = applyJob.category;
          application.jobType = applyJob.jobType;
        }
      }
      res.send(result);
    });
